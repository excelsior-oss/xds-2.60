...
BEGIN
  ...
  bytes := pc.code.get_size(pc.su_bytes,t);
  IF bytes > 4 THEN 
    env.config.Equation("INT64SUFFIX", str);
    out.ws(str^); 
  ELSIF bytes > 2 THEN 
    out.wr("L"); 
  END;
...
BEGIN
  ...
  FOR i:=0 TO prm_cnt-1 DO
    ...
    ELSE
      ...
      IF prm_buf[i].mode = pm_param THEN
        IF ((t.obj # NIL) & ~t.obj.is_public() & (pc.otag_RO IN prm_buf[i].obj.tags))
        THEN
          out.ff(str, "const %s", buf);
          buf := str;
        END;
      END;
    END;
