...
VAR
  tmp: PCOMPONENT_BREAK;
  i : CARDINAL;

BEGIN
  WITH ComponentBreaks DO
    xStr.Uppercase(CB.CompName^);
    fil.RemoveExtension(CB.CompName^);
    IF (CB.CompName^[LENGTH(CB.CompName^) - 1] # '$') THEN
      xStr.Append("$", CB.CompName^);
    END;
    IF ((ComponentBreaks # NIL) AND (free # 0)) THEN
      FOR i := 0 TO free-1 DO
        IF (ComponentBreaks^[i].CompName^ = CB.CompName^) THEN
          RETURN TRUE;
        END;
      END;
    END;
    IF ComponentBreaks = NIL THEN
      NEW(ComponentBreaks, HN);
      IF ComponentBreaks = NIL THEN RETURN FALSE; END;
    ELSIF free > HIGH(ComponentBreaks^) THEN
      NEW(tmp, HN+HIGH(ComponentBreaks^)+1);
      IF tmp = NIL THEN RETURN FALSE; END;
      sys.MOVE(sys.ADR(ComponentBreaks^[0]), sys.ADR(tmp^[0]), (HIGH(ComponentBreaks^)+1)*SIZE(CB));
      DISPOSE(ComponentBreaks);
      ComponentBreaks := tmp;
    END;
    ComponentBreaks^[free] := CB;
    INC(free);
  END;
  RETURN TRUE;
END Add_ComponentBreak;


PROCEDURE Delete_ComponentBreak (BreakPos: CARDINAL): BOOLEAN;
BEGIN
  WITH ComponentBreaks DO
    IF (ComponentBreaks = NIL) OR (free <= BreakPos) THEN RETURN FALSE; END;
    WITH ComponentBreaks^[BreakPos] DO
      IF CompName # NIL THEN xStr.dealloc_str(CompName); END;
    END;
    IF BreakPos < free-1 THEN
      sys.MOVE(sys.ADR(ComponentBreaks^[BreakPos+1]),
               sys.ADR(ComponentBreaks^[BreakPos]),
               SIZE(COMPONENT_BREAK)*(free-BreakPos));
    END;
    DEC(free);
  END;
  RETURN TRUE;
END Delete_ComponentBreak;
...
VAR
  i: CARDINAL;
BEGIN
  FOR i := 1 TO ComponentBreaks.free DO
    IF NOT Delete_ComponentBreak(0) THEN RETURN FALSE; END;
  END;
  RETURN TRUE;
END DeleteAll_ComponentBreak;
...
BEGIN
  ...
  WITH ComponentBreaks DO
    IF ComponentBreaks # NIL THEN
      FOR i := 0 TO HIGH(ComponentBreaks^) DO
        IF Delete_ComponentBreak (i) THEN END;
    ...
    END;
  END;
  ComponentBreaks := COMPONENT_BREAKS{ NIL, 0};
