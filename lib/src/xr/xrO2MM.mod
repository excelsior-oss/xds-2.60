...
BEGIN
  rts.X2C_PrepareToGC;
<* IF DEFINED(ALLOC_PROFILE) & ALLOC_PROFILE THEN *>
  p.sprintf (printBuffer, "BEFORE GC: usedmem = %d K, busymem = %d K, maxmem = %d K\n",
...
<* END *>
  ...
  xrMM.UsedMemAtGCStart := rts.X2C_usedmem;
  -- ASSERT (xrMM.UsedMemAtGCStart >= xrMM.UsedMemAtGCEnd);
<* IF NOT __GEN_C__ THEN *> -- GCInfo engine uses INT64 
  IF xrMM.UsedMemAtGCStart > xrMM.UsedMemAtGCEnd THEN
    INC(xrMM.TotalBytesAllocated, (xrMM.UsedMemAtGCStart - xrMM.UsedMemAtGCEnd));
  END;
<* END *> -- GCInfo engine uses INT64 
  ...
  IF (curTime >= GCStartTime) THEN
    ...
    INC(TotalGCTime, GCPeriod);
  END;
  MutatorStartTime := curTime;

<* IF DEFINED(ALLOC_PROFILE) & ALLOC_PROFILE THEN *>
  p.sprintf (printBuffer, "AFTER GC: usedmem = %d K, busymem = %d K, maxmem = %d K\n",
            rts.X2C_usedmem DIV 1024, rts.X2C_busymem DIV 1024, rts.X2C_maxmem DIV 1024);
  xrtsOS.X2C_StdOutS (printBuffer, 0);
<* END *>
  ...
  xrMM.UsedMemAtGCEnd := rts.X2C_usedmem;
  -- Note: O2 destructors called during GC can allocate memory and increase usedmem,
  -- so we cannot assume that usedmem is decreased during GC.
<* IF NOT __GEN_C__ THEN *> -- GCInfo engine uses INT64 
  IF (xrMM.UsedMemAtGCEnd < xrMM.UsedMemAtGCStart) THEN
    INC (xrMM.TotalBytesReclaimed, (xrMM.UsedMemAtGCStart - xrMM.UsedMemAtGCEnd));
  END;
<* END *> -- GCInfo engine uses INT64 
...
<* IF DEFINED(ALLOC_PROFILE) & ALLOC_PROFILE THEN *>
  ...
  p.sprintf (printBuffer, "AFTER ADAPTIVE HL: usedmem = %d K, busymem = %d K, maxmem = %d K\n\n",
             rts.X2C_usedmem DIV 1024, rts.X2C_busymem DIV 1024, rts.X2C_maxmem DIV 1024);
  xrtsOS.X2C_StdOutS (printBuffer, 0);

  p.sprintf (printBuffer, "GC Took: %d ms\n", curTime - GCStartTime);
  xrtsOS.X2C_StdOutS (printBuffer, 0);
<* END *>
