...
BEGIN
    ...
    | t_int,
      ...
      t_ZZ :        r := pc.value.new (xiEnv.null_pos, pc.ZZ_type);
                    ...
                    IF p.is_ZZ() THEN
                      sav_r := pc.value.new (xiEnv.null_pos, pc.ZZ_type);
                      sav_r.unary (pc.su_conv, r);

                      IF tt=t_int THEN correctSigned(r,fl);
                      ELSE             correctUnsigned(r,fl);
                      END;
                      -- check correctness -- see issue XDS-25
                      cmpTmp.binary (pc.sb_equ, sav_r, r);
                      IF cmpTmp.is_zero () THEN
                        xiEnv.errors.Fault(xiEnv.null_pos,450,
                          "Internal fault: Erroneous compiler behaviour with VAL constant calculation");
                      END;
                    ELSIF ~p.is_RR() THEN
                      IF tt=t_int THEN correctSigned(r,fl);
                      ELSE            correctUnsigned(r,fl);
