...
IMPORT  SYSTEM, X2C, RTS:=xmRTS, xrMM, xrO2MM, xrBlockManager;
...
VAR
  totalAlloc :SYSTEM.INT64;
BEGIN
  totalAlloc := xrMM.TotalBytesAllocated;

  -- add memory allocated since last GC
  IF (RTS.X2C_usedmem > xrMM.UsedMemAtGCEnd) THEN
    INC(totalAlloc, (RTS.X2C_usedmem - xrMM.UsedMemAtGCEnd));
  END;

  gcInfo.mbAllocated := CARDINAL(totalAlloc DIV (1024*1024));
  gcInfo.mbReclaimed := CARDINAL(xrMM.TotalBytesReclaimed DIV (1024*1024));
  gcInfo.totalGCTime := xrO2MM.TotalGCTime;
END getGCInfo;
