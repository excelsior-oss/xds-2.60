...
void	tab_click (MDIDATA * m)
{
	...
	child = tab_window (m, i);
	if (! child) return;
	SendMessage (m->FrameWindow, WM_MDIACTIVATE, (WPARAM) child, 1);
}
...
HWND	next_nonmin_window (MDIDATA * m, HWND hwnd)
{
	int i, i0;
	...
	for (i = i0+1; i < m->win_cnt; i++)
		if (m->win_list [i] && !IsIconic (m->win_list [i])) return m->win_list [i];
	for (i = 0; i < i0; i++)
		if (m->win_list [i] && !IsIconic (m->win_list [i])) return m->win_list [i];
	return NULL;
}
...
void	activate_child (MDIDATA * m, HWND hwnd)
{
	...
	if (m->Tab)	{
		i = tab_number (m, m->active_child);
		PostMessage (m->Tab, TCM_SETCURSEL, i, 0);
		/* Using TabCtrl_SetCurSel or SendMessage doesn't work
		   if tab position changes in response to a tab click.
		   This may happen with Classic MDI model
		*/
	}
}
...
LRESULT	CALLBACK MyMDIClientProc (HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_MYMDI_SIZE:
		...
		if (wparam == SIZE_MINIMIZED)	{
			...
			child = next_nonmin_window (m, child);
			if (child)
				activate_child (m, child);
...
void	Frame_paint (MDIDATA * m)
{
	...
	DeleteObject (br);
	if (m->Tab)	{
		GetWindowRect (m->Tab, &rc);
		pt.x = 0; pt.y = rc.top;
		ScreenToClient (m->FrameWindow, &pt);
		rc.bottom = pt.y;
		rc.top = m->FrameArea.bottom;
		rc.left = 0;
		rc.right = m->FrameArea.right;
		if (rc.bottom > rc.top)	{
			light =	CreatePen (PS_SOLID, 1, GetSysColor (COLOR_3DHIGHLIGHT));
			pen = SelectObject (dc, light);
			MoveToEx (dc, 0, rc.top, 0);
			LineTo (dc, rc.right, rc.top);
			SelectObject (dc, pen);
			DeleteObject (light);
			ExcludeClipRect (dc, 0, rc.top, rc.right, rc.top+1);
			rc.top ++;
			br = CreateSolidBrush (GetSysColor (COLOR_3DFACE));
			FillRect (dc, &rc, br);
			DeleteObject (br);
	...
	if (m -> Tab)	{
		...
		if (m->tab_attr & MDITAB_TOP)	{
			...
			MoveWindow (m->Tab, m->FrameArea.left, m->FrameArea.bottom,
				    m->FrameArea.right-m->FrameArea.left,
				    rc.top - m->FrameArea.top, 1);
				    m->FrameArea.bottom -= 3;
		}
		InvalidateRect (m->FrameWindow, 0, 0);
	}
	Rearrange_dock (m);
	set_min_positions (m);
	test_draw_scrollbars (m, TRUE);
...
LRESULT	MyDefMDIProc (HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_PARENTNOTIFY:
		...
		return 0;
	case WM_NCDESTROY:	RemoveProp (hwnd, MAKEINTATOM (mdi_prop_atom)); break;
	case WM_MDICREATE:
	case WM_MDICASCADE:
	case WM_MDITILE:
	case WM_MDIICONARRANGE:
	...
	case WM_NOTIFY:
		nm = (NMHDR*) lparam;
		if (nm->code == NM_CLICK && nm->hwndFrom == m->Tab)	{
			tab_click (m);
			return 0;
