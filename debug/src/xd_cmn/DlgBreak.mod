...
BEGIN
  ...
  N3 := N2 + brk.ConditionBreaks.free;
  N  := N3 + brk.ComponentBreaks.free;
  w := men.SkipMenus();
  IF N = 0 THEN
  ...
  ELSIF w = std.Wnds[std.BrkWindow].hwnd THEN
    IF mode # act.mode_check THEN
      ...
      WITH p2^ DO
        ...
        ELSE
          ASSERT(brk.Delete_ComponentBreak(curr-N2));
        END;
      END;
    END;
  ELSE
...
VAR
  Curr  : CARDINAL;
  Dialog: BOOLEAN;
  BCompCreatedStr : std.MESSAGE;

PROCEDURE AddNewCompCreatedBreak(CompCreatedStr- : ARRAY OF CHAR);
VAR
  CB : brk.COMPONENT_BREAK;
BEGIN
  CB := brk.EmptyCOMPONENT_BREAK;
  WITH CB.Break DO
    Active := TRUE;
    Sticky := TRUE;
    Pass := 0;
    Owner := brk.Dialog;
  END;
  xs.alloc_from(CB.CompName, CompCreatedStr);
  ASSERT(brk.Add_ComponentBreak(CB));
  RefreshBreaks;
END AddNewCompCreatedBreak;

PROCEDURE SetCompCreatedBreak(hwnd: win.HWND):BOOLEAN;
BEGIN
  IF (Curr = MAX(CARDINAL)) THEN
    AddNewCompCreatedBreak(BCompCreatedStr);
  ELSE
    WITH brk.ComponentBreaks.ComponentBreaks^[Curr] DO
      xs.dealloc_str(CompName);
      xs.alloc_from(CompName, BCompCreatedStr);
    END;
    RefreshBreaks();
  END;
  deve.AddToTail(hwnd, deve.Hide, 0);
  deve.Flush;
  RETURN TRUE;
END SetCompCreatedBreak;


PROCEDURE CompCreatedPoint (action: act.ACTION; mode: act.ACTION_MODE): BOOLEAN;
BEGIN
  ASSERT(action = act.CompCreatedPoint);
  IF mode # act.mode_check THEN
    Curr := MAX(CARDINAL);
    std.OpenUniversalDialog(mes.dlg_CompCreatedBreakpoint, SetCompCreatedBreak, BCompCreatedStr);
  END;
  RETURN TRUE;
END CompCreatedPoint;
...
VAR
  ...
  PROCEDURE ComponentBreaks (): BOOLEAN;
  VAR
    i: CARDINAL;
    r: BOOLEAN;
  BEGIN
    WITH brk.ComponentBreaks DO
      IF free = 0 THEN RETURN FALSE; END;
      IF mode = act.mode_check THEN
        r := FALSE;
        FOR i := 0 TO free-1 DO
          CASE action OF
          | act.EnableAll:
            r := r OR NOT ComponentBreaks^[i].Break.Active;
          | act.DisableAll:
            r := r OR ComponentBreaks^[i].Break.Active;
          END;
        END;
        RETURN r;
      ELSE
...
BEGIN
  ...
  IF mode = act.mode_check THEN
    RETURN Breakpoints() OR Accesss() OR ConditionBreaks() OR ComponentBreaks();
  ELSE
    r := Breakpoints();
    r := Accesss() AND r;
    r := ConditionBreaks() AND r;
    r := ComponentBreaks() AND r;
    RETURN r;
  END;
...
BEGIN
  ...
  IF (brk.Breakpoints.free = 0) AND
     (brk.AccessBreaks.free = 0) AND
     (brk.ConditionBreaks.free = 0) AND
     (brk.ComponentBreaks.free = 0) THEN
    RETURN FALSE;
  END;
  IF mode # act.mode_check THEN
<* PUSH *>
...
<* POP *>
    ...
    ASSERT(brk.DeleteAll_ComponentBreak());
    deve.AddToTail(std.Wnds[std.MainWindow].hwnd, deve.Redraw, 0);
    WITH std.Wnds[std.BrkWindow] DO
      IF (hwnd # win.Invalid_H) AND (win.Visible(hwnd)) THEN
        deve.AddToTail(hwnd, deve.Redraw, 0);
  ...
  BEGIN
    WITH p^ DO
      ...
      ELSIF (num < N3) THEN
        ...
        END;
      ELSIF (num < N) THEN
        WITH brk.ComponentBreaks.ComponentBreaks^[num - N3] DO
          pro.GetMsg(mes.CompCreatedBreak, str);
          fmt.print(buf, "Component: %s", CompName^);
          IF NOT Break.Active THEN
            attr := crt.Attr(crt.Fg(crt.Src[crt.Src_BreakDisabled]), crt.Bg(attr));;
          END;
          crt.WrStrFromPos(hwnd, buf, attr, pos);
      ...
      | act.Enable, act.Disable:
        IF N > 0 THEN
          ...
          ELSIF curr < N3 THEN
            ...
            END;
          ELSE
            WITH brk.ComponentBreaks.ComponentBreaks^[curr-N2].Break DO
              IF (action = act.Disable) THEN
                act.ConfirmQueryByCond(Active);
              ELSE
                act.ConfirmQueryByCond(NOT Active);
     ...
     <* IF DEST_XDS THEN *>
      | key.Enter, key.AltEnter:
        ...
        ELSIF curr < N THEN
          ...
          END;
          Curr := curr - N3;
          COPY (brk.ComponentBreaks.ComponentBreaks^[Curr].CompName^, BCompCreatedStr);
          std.OpenUniversalDialog(mes.dlg_CompCreatedBreakpoint, SetCompCreatedBreak, BCompCreatedStr);
        END;
<* END *>
      ELSE
...
BEGIN
  ...
  WITH brk.ComponentBreaks DO
    IF free > 0 THEN
      FOR i := 0 TO free-1 DO
        WITH ComponentBreaks^[i] DO
          COPY(CompName^, comp_name);
          out ('  BREAK compbrk%-2i, COMP, %-15s, %s\n', i+1, comp_name, dlt.Switch_To_Dialog);
        END;
      END;
      out ('\n');
    END;
...
BEGIN
  ...
  act.IniAction(act.CompCreatedPoint,   CompCreatedPoint);
