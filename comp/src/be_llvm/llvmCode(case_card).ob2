...
BEGIN
  ...
  | ir.y_NumConst:
      IF p.triade.Op = ir.o_call THEN
        nms.GetProcParamType( type_name, p.triade.Prototype, p.protoparnum
                            , GetProcObj(p.triade.Params[0]) );  
      ELSE
        nms.GetParamTypeName(type_name, p);
      END;
      nms.ValueToStr(value_name, p.value);
      res := ei.NewArg(value_name, type_name, 0);
  ...
  | ir.y_RealConst:
      ...
      ELSE
        nms.GetParamTypeName(type_name, p);
      END;
      nms.RealValueToStr(value_name, p.value, IsTriadeParamSingle(p));
      res := ei.NewArg(value_name, type_name, 0);
...
VAR x: ei.Arg;
    i, j, varinats_num: LONGINT;
    case_label, else_label: def.LabelName;
    case_value: nms.String;
    case_start, case_end: LONGINT;
    case_start_u, case_end_u, k: sys.CARD32;
...
BEGIN
  ...
  FOR i := varinats_num - 1 TO 0 BY -1 DO
    ...
    IF tr.OpType = ir.t_int THEN
      case_start := nms.Par2Integer(tr.Params[(2*i) + 1]);
      case_end   := nms.Par2Integer(tr.Params[(2*i) + 2]);
      FOR j := case_end TO case_start BY -1 DO
        nms.IntToStr(case_value, j); 
        case_list := ei.NewCaseVariant(case_value, case_label, case_list);
      END;
    ELSE
      case_start_u := nms.Par2Cardinal(tr.Params[(2*i) + 1]);
      case_end_u   := nms.Par2Cardinal(tr.Params[(2*i) + 2]);
      k := case_end_u;
      IF case_start_u = case_end_u THEN
        nms.CardToStr(case_value, k); 
        case_list := ei.NewCaseVariant(case_value, case_label, case_list);
      ELSE
        WHILE k >= case_start_u DO
          nms.CardToStr(case_value, k); 
          case_list := ei.NewCaseVariant(case_value, case_label, case_list);
          k := k - 1;
        END;
  ...
  ei.GenSwitch(x, else_label, case_list)
END GenCase;
...
BEGIN
  ...
  IF (obj # NIL) & (obj.type # NIL) THEN
    param := obj.type.prof;
    FOR i := 0 TO LEN(arg_list^)-1 DO
      CASE  proto.par[i].mode OF
      | prc.pm_param:
          IF (param.type.mode = pc.ty_array_of) THEN
            nms.GetTypeName(tname, param.type);
  ...
  ELSE  
    FOR i := 0 TO LEN(arg_list^)-1 DO
      IF (proto.par[i].type = ir.t_ref) 
       & (tr.Params[i+1].type # NIL) & (tr.Params[i+1].type.mode IN pc.ARRs)
      THEN
...
BEGIN
  ...
  obj := GetProcObj(tr.Params[0]);
  ObjNames.makename(obj, proc_name);
