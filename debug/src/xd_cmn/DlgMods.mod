...
PROCEDURE AddCompBreak (action: act.ACTION; mode: act.ACTION_MODE): BOOLEAN;
VAR
  p: std.PLIST;
  w : win.HWND;
  N : CARDINAL;
BEGIN
  ...
  IF (N = 0) THEN
    ...
    RETURN FALSE;
  END;
  IF w = std.Wnds[std.ComponentsWindow].hwnd THEN
    IF mode # act.mode_check THEN
      ASSERT(action = act.AddCompBreak);
      p := win.GetAMPtr(w);
      dbrk.AddNewCompCreatedBreak(dt.Components.Components^[p^.curr].EI.short_name);
    END;
  ELSE
  ...
  WITH std.Wnds[std.ComponentsWindow] DO
    IF hwnd = win.Invalid_H THEN
      ...
      WITH p^ DO
        ...
        actions[1] := act.CONTEXT{ act.do_action, act.AddCompBreak};
...
BEGIN
  ...
  act.IniAction(act.AddCompBreak, AddCompBreak);
END DlgMods.
