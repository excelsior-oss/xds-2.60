...
--  ReOpenBuff : win.OFSTRUCT;
 ...
 <* END *>
  ...
  BEGIN
   <* IF USEFILEMAPPING THEN *>
    hfile    := win.INVALID_HANDLE_VALUE;
    lpFile := NIL;
    hMapFile := NIL;
   <* ELSE *>
    res := rf.otherProblem;
...
BEGIN
 ...
 <* END *>
  WITH CurrComponent^.DI.Modules^[mod] DO
   ...
   <* IF USEFILEMAPPING THEN *>
    hfile := win.CreateFile(dbgfullfname, 
                            win.GENERIC_READ, win.FILE_SHARE_READ, NIL, win.OPEN_EXISTING, win.FILE_ATTRIBUTE_NORMAL, NIL);
   
    IF hfile = win.INVALID_HANDLE_VALUE THEN
      <* IF TESTEDIF THEN *>
      printf("[!] win.OpenFile(%s) failed, win.GetLastError = %d\n", dbgfullfname, win.GetLastError());           
      <* END *>
      ods.AddOutputDebugString ("Cannot open dbg file %s (location %s), win.GetLastError = %d", dbgfname, dbgfullfname, win.GetLastError());
      RETURN;
    END;
    ...
    IF hMapFile = NIL THEN
      <* IF TESTEDIF THEN *>
      printf("[!] Cannot create mapping dbg file %s (location %s): win.GetLastError = %d\n", dbgfname, dbgfullfname, win.GetLastError());           
      <* END *>
    ...
    IF lpFile = NIL THEN
      <* IF TESTEDIF THEN *>
      printf("[!] win.MapViewOfFile failed for dbg file %s (location %s): win.GetLastError = %d\n", dbgfname, dbgfullfname, win.GetLastError());           
      <* END *>
      ods.AddOutputDebugString ("Cannot map dbg file %s (location %s)", dbgfname, dbgfullfname);
      RETURN;
    END;
    RawDebugInfo := sys.ADDRESS(lpFile);
    DebugSize := win.GetFileSize  (hfile, NIL);
    IF DebugSize = 0FFFFFFFFH THEN
      <* IF TESTEDIF THEN *>
      printf("[!] Invalid size dbg file %s (location %s): win.GetLastError = %d\n", dbgfname, dbgfullfname, win.GetLastError());           
      <* END *>
