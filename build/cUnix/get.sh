...
for file_name in build.sh makexds-c.sh pack-xds.sh setenv.sh
do
  chmod a+x "$file_name"
done
...
then
  for file_name in build.sh makexds-c.sh pack-xds.sh setenv.sh
  do
    CopyFiles "build/cUnix/$file_name"  ""  || exit $?
  done
...
RemoveFiles "enduser/xds/bin/xc*"

# ============================================= API 
CopyFiles "api/enduser/linux/*-c"     "enduser/xds/"  || exit $?
CopyFiles "api/enduser/linux/setenv"  "enduser/xds/"  || exit $?
...
# ============================================= Licence files  
CopyFiles "misc/licenses/linux/licenses.txt"  "enduser/xds/licenses/"  || exit $?
CopyFiles "misc/licenses/linux/xdsc.txt"      "enduser/xds/licenses/"  || exit $?
...
RemoveFiles "enduser/xds/pdf/xc*"
