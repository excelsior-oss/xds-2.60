...
class CFile : public OSFile {
  ...
  public:
    ...
    virtual Bool OpenRead (const char * filename, byte * &data, unsigned long &size)
    {
        ...
        size = ftell (fp);
        if (size == 0) {
            data = (byte *) xalloc (1); // just to return non-null pointer
    ...
    virtual dword getFileAttributes (const char * filename)
    {
        ...
        if (stat(filename, &sb) == 0) {
            ...
            if (access(filename, W_OK) == 0) {
                attr |= file_attribute_writeable;
            }
            if (access(filename, X_OK) == 0) {
                attr |= file_attribute_executable;
            }
  ...
  public:
    ...
    virtual dword getFileAttributes (const char * filename)
    {
        ...
        if (stat(filename, &sb) == 0) {
            ...
            // FIXME Add check for writeable and executable flags
        }
        return attr;
    }
...
class Win32MappingFile : public OSFile {
  ...
  public:
    ...
    virtual Bool OpenRead (const char * filename, byte * &data, unsigned long &size)
    {
        ...
        if (FileInformation.nFileSizeLow == 0) {
            lpFileBase = xalloc (1); // just to return non-null pointer
...
class Win32_Interface : public OSInterface {
  public:
    ...
    virtual dword getFileAttributes (const char * filename)
    {
        ...
        if (a != 0xffffffff) {
            return file_attribute_exists | file_attribute_readable | file_attribute_executable |
                   (!(a & FILE_ATTRIBUTE_READONLY) ? file_attribute_writeable : 0) |
