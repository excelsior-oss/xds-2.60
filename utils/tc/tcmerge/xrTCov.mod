...
BEGIN
  ...
  IF xlibOS.X2C_Exists (newfname) THEN
    xlibOS.X2C_Remove (newfname);
  END;
  IF xlibOS.X2C_Rename (fname, newfname) # 0 THEN
    Error (msgMakeBackup, newfname);
  END;

(*
  i := 1;   
  WHILE xlibOS.X2C_Rename (fname, newfname) # 0 DO
    pos := 0;
    xrFName.X2C_ExtractBaseName (fname, newfname);
...
VAR ini, res: xlibOS.X2C_TimeStruct;
BEGIN
  writeDec (file, time, 0);
  writeS   (file, ' ', 1);

  xlibOS.X2C_GetTime (ini);
  ini.year  := 1970;
  ini.month := 1;
  ini.day   := 1;
  ini.hour  := 0;
  ...
  ini.sec   := 0;
  ini.fracs := 0;
  xlibOS.X2C_TimeSecAdd (ini, time, res);
...
BEGIN
  ...
  IF xlibOS.X2C_Exists (fname) THEN
