...
class EFS_IR_File {
  private:
    ...
    void readExtraAttrs (TextFileParser* parser);
...
void EFS_IR_File :: read (TextFileParser * parser, EFSDataFile* dataFile) {
    ...
    if (packed) {
        ...
        if ((osAttrs & file_attribute_writeable) != 0) {
            attrs |= fs_writeable;
        }
        if ((osAttrs & file_attribute_executable) != 0) {
            attrs |= fs_executable;
        }
...
void EFS_IR_File :: readExtraAttrs (TextFileParser* parser) {
    for (;;) {
        parser -> skipSpaces ();
        char * extAttr = parser->getToken();
        if (extAttr == NULL) {
            return;
        }

        if (!strcmp(extAttr, "HIDDEN")) {
            attrs |= fs_notiteratable;

        } else if (!strcmp(extAttr, "EXTRA")) {
            parser -> skipSpaces ();
            dword nBytes = parser -> getHex();

            if (nBytes != 0) {
                extra = new byte[nBytes];
                extra_length = nBytes;

                for (dword i = 0; i < nBytes; i++) {
                    parser -> skipSpaces ();
                    extra[i] = (byte) parser -> getHex();
                }
            }

        } else {
            parser -> expect ("HIDDEN or EXTRA"); // fatal error
            ASSERT_FALSE();
        }
    }
}
...
void EFS_IR_File :: write (dword efile_offs) {
    ...
    if (extra != NULL) {
        dword extra_offs = EFSDirStorage->GetPos();

        addFixup (EFSDescSeg,
                  FIXUP_ADDRESS32,
                  efile_offs + offsetof (struct EFile, extra),
                  TK_SEG,
                  EFSDirSeg,
                  extra_offs);

        EFSDirStorage -> Put4(extra_length);
        EFSDirStorage -> PutS(extra, extra_length);
    }
...
void EFS_IR_File :: serialize (FILE * f, int identation) {
    fprintf (f, "%*c%s \"%s\" %x %x %x", identation, ' ',
             ((dir == NULL) ? "PACKED_FILE" : "PACKED_DIR"),
             efsFileName, attrs, lastModifiedTimeLO, lastModifiedTimeHI);

    if (dir == NULL) {
        fprintf (f, " %x %x", data_offs, length);
    }

    if (extra != NULL) {
        fprintf (f, " EXTRA %x", extra_length);
        for (dword i = 0; i < extra_length; i++) {
            fprintf (f, " %x", extra[i]);
        }
    }
