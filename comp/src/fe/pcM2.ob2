...
(** M2 parser *)
<* IF ~ DEFINED(TARGET_TESTCOVERAGE) THEN *> <* NEW TARGET_TESTCOVERAGE- *> <* END *>
...
BEGIN
  ...
  LOOP
    ...
    END;
  END;
  IF pcS.warn_not_translated & (q.mno # pc.cur_mod) THEN
    q := v;
    LOOP
      IF (q.mode=pc.ob_module) THEN
        IF (q.name^="SYSTEM") THEN
          IF NOT pcS.isAllowedSystemUse(v.name^) THEN
            IF (v.name^="ADDRESS") THEN
              pcS.warn(4031);
            ELSIF (v.name^="ADR") THEN
              pcS.warn(4032);
            ELSIF (v.name^="M2ADR") THEN
              pcS.warn(4033);
            ELSIF (v.name^="BYTE") THEN
              IF NOT pcS.disable_system_byte_warn THEN
                pcS.warn(4034);
              END;
            ELSIF (v.name^="FIELDOFS") THEN
              pcS.warn(4035);
            ELSIF (v.name^="GET") THEN
              pcS.warn(4036);
            ELSIF (v.name^="MOVE") THEN
              pcS.warn(4037);
            ELSE
              pcS.warn(4030, v.name^);
            END;
          END;
        ELSIF pcS.isLibraryModule(q.name^) THEN
          pcS.warn(4005, q.name^);
          EXIT;
        END;
      END;
      IF (q.host = NIL) THEN
...
BEGIN
  ...
  IF v.mode=pc.ob_type THEN
    IF pcS.warn_not_translated THEN
      IF (v.type.mode = pc.ty_char) THEN
        pcS.warn(4006);
      ELSIF (v.type.mode IN pc.TY_SET{pc.ty_shortcard, pc.ty_cardinal, pc.ty_longcard, pc.ty_longlongcard}) THEN
        pcS.warn(4007);
      END;
    END;
...
-- parse call chain: a.x().y()...z()
PROCEDURE eat_spaghetti(VAR n: pc.NODE; start_pos: pc.TPOS; need_void: BOOLEAN): BOOLEAN;
VAR
  proc: pc.NODE;
  is_void: BOOLEAN;
BEGIN
  IF pcS.oberon & pcS.ext() & (env.o2_spaghetti IN env.config.tags) THEN
    IF (sy # pcS.lpar) OR (n.mode # pc.nd_method) THEN
      RETURN FALSE; -- there are no call chains, try to parse something else
    END;
  ELSE
    RETURN FALSE;
  END;

  -- ok, try to parse something...
  WHILE (sy = pcS.lpar) & (n.mode = pc.nd_method) DO
    proc := n;
    is_void := (proc.type.base.mode = pc.ty_void);
    pcO.new(n, pc.nd_call);
    n.pos := start_pos;
    n.l := proc;
    n.type := pcO.invtype;
    ProcCall(n, ~need_void OR ~is_void);
    n.end := pcS.txtpos;
    ...
    IF need_void & (sy # pcS.lpar) & ~is_void & ~(proc.type.flag IN pc.CallResOptional)
    THEN -- call chain ends by non-void function in the operator (non-expression) context
      pcS.error(n.pos,'e',90);
      n.type:=pcO.invtype;
      RETURN TRUE;
    END;
  END;
...
PROCEDURE Expr(VAR e1: pc.NODE);
  ...
  BEGIN
    ...
    designator(n);
    IF eat_spaghetti(n, start_pos, FALSE) THEN
      -- ok, call chain is eaten
    ELSIF n.mode=pc.nd_sproc THEN
      sProcCall(n);
...
VAR
  ...
  BEGIN
    ...
    IF pcS.warn_not_translated THEN
      IF (type.mode IN pc.TY_SET{pc.ty_array,pc.ty_array_of,pc.ty_record}) THEN
        pcS.warn(4025);
      END;
    END;
    x.l:=n;
    ...
    CASE sy OF
     |pcS.ident :
        ...
        ELSIF i.type.mode = pc.ty_proctype THEN
          x := i;
          IF eat_spaghetti(x, start_pos, TRUE) THEN
            -- eaten successfully
          ELSE
...
BEGIN
  ...
  IF sy#pcS.rpar THEN
    LOOP
      ...
      WHILE l#NIL DO
        ...
        IF pcS.warn_not_translated THEN
          IF (l.mode = pc.ob_varpar) THEN
            IF NOT (parm.mode IN pc.TY_SET{pc.ty_array,pc.ty_array_of,pc.ty_record}) THEN
              pcS.warn(4009);
            END;
            IF (parm.mode IN pc.TY_SET{pc.ty_array,pc.ty_array_of}) &
               (parm.base.mode = pc.ty_char)
            THEN
              pcS.warn(4010);
            END;
          ELSIF (pc.otag_RO IN l.tags) THEN
            -- RO parameters are of any type are allowed
          ELSE
            IF (NOT pcO.def) & (parm.mode IN pc.TY_SET{pc.ty_array,pc.ty_array_of,pc.ty_record}) THEN
              pcS.warn(4011);
            END;
          END
        END;
    ...
    IF sy=pcS.seq THEN
      ...
      ELSE
        ...
        IF pcS.warn_not_translated THEN
          -- allow SYSTEM.BYTE for SEQ parameters definition
          pcS.disable_system_byte_warn := TRUE;
        END;
        IF pcS.oberon THEN type_definition(parm,FALSE)
        ELSE type_qualident(p,FALSE); parm:=p.type;
        END;
        IF pcS.warn_not_translated THEN
          pcS.disable_system_byte_warn := FALSE;
        END;
  ...
  IF sy=pcS.colon THEN
    ...
    END;
    IF pcS.warn_not_translated THEN
      IF (parm.mode IN pc.TY_SET{pc.ty_array,pc.ty_array_of,pc.ty_record}) THEN
        pcS.warn(4026);
      END;
    END;
    type.base:=parm;
...
PROCEDURE type_definition(VAR t: pc.STRUCT; en_array_of: BOOLEAN);
    ...
    BEGIN
      ...
      IF pcS.warn_not_translated THEN
        IF (t.mode = pc.ty_record) THEN
          pcS.warn(4023);
        ELSIF (t.mode IN pc.TY_SET{pc.ty_array, pc.ty_array_of}) THEN
          pcS.warn(4024);
        END;
      END;
      WHILE l#NIL DO
  ...
  BEGIN
    ...
    END;
    IF pcS.warn_not_translated THEN
      IF (t.base.mode IN pc.TY_SET{pc.ty_record, pc.ty_array, pc.ty_array_of}) THEN
        pcS.warn(4015);
      END;
    END;
...
BEGIN
  ...
  CASE sy OF
   ...
   |pcS.procedure: IF pcS.warn_not_translated THEN
                     pcS.warn(4016);
                   END;
                   IF pcS.oberon THEN o2_proctype(t) ELSE m2_proctype(t) END;
...
--    type.flag:=sf;
    ...
    IF pcS.warn_not_translated & (p.lev > 0) THEN
      pcS.warn(4017)
    END;
