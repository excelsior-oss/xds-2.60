...
VAR
 ...
 (** compilation modes *)
  ...
  warn_not_translated -: BOOLEAN; -- warn about constructs which
                                  -- cannot be translated to Java

  disable_system_byte_warn* :BOOLEAN; -- disable warnings about SYSTEM.BYTE

  allowed_cond_comp   :env.String;  -- list of allowed conditional compilations
  allowed_system_uses :env.String;  -- list of allowed uses of module SYSTEM
  library_modules     :env.String;  -- list of library modules
  reserved_identifiers:env.String;  -- list of reserved identifiers
...
PROCEDURE isListed(name -: ARRAY OF CHAR; list :env.String) :BOOLEAN;
VAR
  p1, p2, len1, len2 :LONGINT;
BEGIN
  IF (list = NIL) THEN
    RETURN FALSE;
  END;
  ...
  LOOP
    p2 := 0;
    WHILE (p1 < len1) & (p2 < len2) & (list[p1] = name[p2]) DO
      INC(p1); INC(p2);
    END;
    IF (p2 = len2) & ((p1 = len1) OR (list[p1] = ';')) THEN
      RETURN TRUE;
    END;
    WHILE (p1 < len1) & (list[p1] # ';') DO
      INC(p1);
    END;
    INC(p1);
    IF (p1 >= len1) THEN
...
PROCEDURE isLibraryModule*(name -: ARRAY OF CHAR) :BOOLEAN;
BEGIN
  RETURN isListed(name, library_modules);
END isLibraryModule;

PROCEDURE isAllowedSystemUse*(name -: ARRAY OF CHAR) :BOOLEAN;
BEGIN
  RETURN isListed(name, allowed_system_uses);
END isAllowedSystemUse;

PROCEDURE isAllowedCondComp() :BOOLEAN;
BEGIN
  RETURN isListed(name, allowed_cond_comp);
END isAllowedCondComp;

PROCEDURE isReservedIdentifier() :BOOLEAN;
BEGIN
  RETURN isListed(name, reserved_identifiers);
END isReservedIdentifier;
...
PROCEDURE If(VAR sy: Symbol; entry: BOOLEAN);
  ...
  PROCEDURE Expr(): BOOLEAN;
    ...
    BEGIN
      IF sy=ident THEN
        IF warn_not_translated THEN
          IF NOT isAllowedCondComp() THEN
            warn (4003,name);
          END;
        END;
...
BEGIN
  ...
  END;
  IF warn_not_translated & isReservedIdentifier() THEN
    warn(4018, name);
  END;
  sy:=ident;
END identifier;
...
BEGIN
  ...
  *)
  ...
  warn_not_translated := env.config.Option("WarnNotTranslated");
  disable_system_byte_warn := FALSE;
  env.config.Equation ("AllowedConditionalCompilations", allowed_cond_comp);
  env.config.Equation ("AllowedSystemUses", allowed_system_uses);
  env.config.Equation ("LibraryModules", library_modules);
  env.config.Equation ("ReservedIdentifiers", reserved_identifiers);
END ini;
...
BEGIN
  ...
  Set(FALSE,"O2SPAGHETTI",env.o2_spaghetti);    (* enable method call chains in Oberon-2 *)
