...
Process:
  ...
  do while lines(in) > 0
    ...
    if ordinal <> '' then do
      ...
      if (object <> "") & (wordpos(module,"X2C x2c rtl SCODE MEVT MCI MAKE IS IN HRESULT FD Animate CommDlg DateTime Header ImageList ListView MonthCal Pager PropSheet TabCtrl TreeView") = 0)  then
        line = name "AS" module"__"object 
      else
        line = name
      if first then do
        first = 0
