...
VAR
  AskToGoToCall: win.HWND;
  GoToCall : BOOLEAN;


PROCEDURE GoToCallHandler(hwnd: win.HWND; msg: deve.MSG);
VAR
  size: crt.SZ;
  x, y: CARDINAL;
  attr: crt.ATTR;
...
BEGIN
  ...
  | deve.Redraw, deve.Paint:
    ...
    WITH size DO
      ...
      ELSE
        attr := p^.Colors^[crt.Msg_Frame];
      END;
      pos1 := (x2-x1) DIV 2 -LENGTH('[ Go to call ]')-1;
      crt.SetPos(pos1, y2-y1-2);
      crt.WrStr(hwnd,'[ Go to call ]', attr);
      IF NOT GoToCall THEN
        attr := p^.Colors^[crt.Msg_Button];
      ELSE
        attr := p^.Colors^[crt.Msg_Frame];
      END;
  ...
  | deve.Mouse_Released:
    ...
    size := win.GetWindowSize(hwnd);
    IF y = size.y2-size.y1-2 THEN
      pos1 := (size.x2-size.x1) DIV 2 -LENGTH('[ Go to call ]')-1;
      pos2 := (size.x2-size.x1) DIV 2 + 2;
      IF (pos1 <= x) AND (x <= pos1+LENGTH('[ Go to call ]')-1) THEN
        GoToCall := TRUE;
        deve.AddToTail(hwnd, deve.Hide, 0);
      ELSIF (pos2 <= x) AND (x <= pos2+LENGTH('[ Continue ]')-1) THEN
        GoToCall := FALSE;
...
PROCEDURE GoToCallDialog (message-: ARRAY OF CHAR);
VAR
  p: std.PPMSG;
  size: crt.SZ;
BEGIN
  IF AskToGoToCall = win.Invalid_H THEN
    AskToGoToCall := win.RegisterWindow (GoToCallHandler, SIZE(std.MSG_REC));
    ASSERT(AskToGoToCall # win.Invalid_H);
    win.SetWindowSize (AskToGoToCall, crt.SZ {13, 7, 67, 10});
    std.CenterWindow (AskToGoToCall);
    win.SetModal (AskToGoToCall);
    win.SetHeaderByStr (AskToGoToCall, 'Component loaded');
    p := win.GetAMPtr (AskToGoToCall);
    p^.Colors := sys.ADR (crt.Error);
    std.InitMessage (AskToGoToCall, 8);
  END;
  p := win.GetAMPtr (AskToGoToCall);
  std.SetMessage (AskToGoToCall, message);
  size := win.GetWindowSize(AskToGoToCall);
  size.x2 := size.x2 + 8;
  win.SetWindowSize(AskToGoToCall, size);
  std.CenterWindow (AskToGoToCall);
  std.CenterMessage (win.GetWindowSize (AskToGoToCall), p^.Msg);
  deve.AddToTail (AskToGoToCall, deve.Rise, 0);
END GoToCallDialog;
...
VAR
  ...
  obj: dt.OBJECT;
  user_code_IP: kt.ADDRESS;
  name, comp_name : xs.String;

BEGIN
  ASSERT (data = NIL);
  ASSERT (eve.LastEvent.Event = eve.CompCreated);
  IF (NOT tls.FindComponentByHandle (eve.LastEvent.Component.Handle, com_by_handle)) THEN
    RETURN
  END;
    ...
    FOR i := 0 TO free-1 DO
      WITH ComponentBreaks^[i] DO
        IF (Break.Active) THEN
          ...
          IF comp_name # "" THEN
            xs.Uppercase (name);
            IF comp_name # name THEN -- <name>.dll ?
              fil.RemoveExtension(name);
              IF comp_name # name THEN -- <name> ?
                xs.Append ("$", name);
                IF comp_name # name THEN -- <name>$ ?
                  flag := FALSE;
                END;
          ...
          IF (flag) THEN
            IF (NOT Break.Sticky) THEN
              ASSERT(brk.Delete_ComponentBreak(i));
            END;
            GoToCallDialog(CompName^);
            deve.Flush;
            IF (GoToCall) THEN
              stk.ResetCallStack;
              stk.ScanCallStack;
              IF stk.SkipProcWithoutDebugInfo (user_code_IP, obj) THEN
                ASSERT(mod.SetNewPosByAddr (user_code_IP));
                dex.PointExecLine(user_code_IP);
              END;
              exe.StopExec;
            END;
            RETURN;
...
BEGIN
  ...
  erc.AddAction (eve.CompCreated, NIL, DoCompCreated);
