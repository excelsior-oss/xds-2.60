...
PROCEDURE ParseType ( n,b: Node
                    ;   t: pc.STRUCT);
(*
*)
BEGIN
  IF t.base # NIL THEN
    ParseType (n, b, t.base)
  END;
...
BEGIN
  WORD_BITS := 32;
  w := f.attr.val.get_integer();
  IF w < WORD_BITS THEN
    o := f.attr.l.l.val.get_integer();
    ASSERT( o < 16 );
...
VAR s: LONGINT;
BEGIN
  s := Size (f) + f.attr.l.val.get_integer();
  RETURN s * (WORD_BITS DIV 8);
END Bytes;

-----------------------------------------------------------------------------

PROCEDURE TieSort ( VAR l: Node
                  ;     n: Node);
(*
*)
VAR q: Node;
...
BEGIN
  ...
  WHILE n # NIL DO
    IF n.mode = m_sl1 THEN
      ...
      n := n.next;
      m.size := Size (m.obj);
      m.offs := m.obj.attr.l.val.get_integer();
      TieSort (q(*=>*), m);
    ELSE
      l := n;
      n := n.next;
...
PROCEDURE PackBitFields ( VAR b: Node
                        ;  offs: LONGINT
                        )      : 	Node;
(*
*)
...
BEGIN
  ...
  WHILE l # NIL DO
    ...
    IF (l.mode IN {m_struct, m_union}) & (l.down = NIL) THEN
      ...
      END;
      l := l.next;
    ELSIF (l.mode IN {m_struct, m_union}) & (l.mode = n.mode)
       OR (n.mode = m_union) & (l.mode = m_struct) & (l.down = l.last)
...
BEGIN
  ...
  WHILE l # NIL DO
    ...
    THEN
      o := l.obj.attr.l.l.val.get_integer();
      s := l.obj.attr.val.get_integer();
...
BEGIN
  ...
  n := Parse (t);
  Optimize (n);
  IF pc.mods[t.mno].type.flag = pc.flag_sl1 THEN
    MakeWords (n);
    Optimize (n);
  END;
  IF n.mode = m_union THEN
    INCL (t.tags, cc.ttag_union)
  END;
  h.node := n;
END Record;
-----------------------------------------------------------------------------
