...
//TOOL         *ptDefValue;                 // Не 0 если это был тул из тулсета
  ...
  void          Copy        (TOOL *pSrc);             // Взять значения
  void          free_all    ();
...
{
  int            cb;                          // Для передачи диалогам
  char           szName       [TOOLNAMELEN];  // Имя категории (пустое => категория игнорируется)
  char           szComment    [50];           // Текст 'сопли'
  ...
  PTOOL          pToolList;                   // Список тулов категории
  ULONG          nLastEdTool;                 // Для редактрования - номер последнего изменявшегося тула

                 TOOLCATEGORY ();                       //
                ~TOOLCATEGORY ();                       //
  BOOL           IsEnabled    ();                       // Enabled?
  BOOL           Execute      (HWND hMsgWnd);           // Запуск (вернет FALSE при ошибке)
  void           Apply        (TOOLCATEGORY *pSrc);     // Добавить тулы, заменить совпадающие
  BOOL           WProfile     (LONG slot, PSZ szKey);
  BOOL           RProfile     (LONG slot, PSZ szKey);
  void           free_all     ();
};
...
public:
                TOOLBOX();
               ~TOOLBOX();
  BOOL          Execute           (LONG lCtrlId); // Тулам сопоставляются идентификаторы контролов TOOL_ID_MIN+<номер категории тула>
  BOOL          PlayTool          (PQMSG pqmsg);  // Если сообщению соответствует тул - попробует выполнить, TRUE; иначе - FALSE
  void          EditTools         ();
  void          ReenableAll       ();
  void          TuneMenu          ();
  ...
  void          free_all();
  BOOL          RProfile          (LONG slot, PSZ pszKey); // FALSE если слишком много тулзов
  void          WProfile          (LONG slot, PSZ pszKey);
private:
  PTOOLCATEGORY apToolCategories[MAXTOOLS];    // Категории тулов (отсутствющие - без имени)
