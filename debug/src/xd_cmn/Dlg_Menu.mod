    ...
    WITH MainMenu[BreaksMenu] DO
      IF hwnd = win.Invalid_H THEN
       <* IF DEST_K26 THEN *>
        ...
        Buttons^[05] := MBUTTON{ enabled, act.CompCreatedPoint,'',  0};
        Buttons^[06] := MBUTTON{ enabled, act.Counter,         'x', 9};
        Buttons^[07] := MBUTTON{ enabled, act.Watchpoint,      'y', 3};
        Buttons^[08] := MBUTTON{ enabled, act.ConditionBreak,  'c', 2};
        Buttons^[09] := MBUTTON{ separator };
        Buttons^[10] := MBUTTON{ enabled, act.Disable,   'p', 1};
        Buttons^[11] := MBUTTON{ enabled, act.Enable,    'n', 2};
        Buttons^[12] := MBUTTON{ enabled, act.Delete,    'f', 3};
        Buttons^[13] := MBUTTON{ separator };
        Buttons^[14] := MBUTTON{ enabled, act.ViewAll,   'd', 8};
        Buttons^[15] := MBUTTON{ enabled, act.DisableAll,'r', 3};
        Buttons^[16] := MBUTTON{ enabled, act.EnableAll, 't', 5};
        Buttons^[17] := MBUTTON{ enabled, act.EraseAll,  ',', 2};
       <* ELSE *>
        NEW(Buttons, 20);
        ...
        Buttons^[05] := MBUTTON{ enabled, act.CompCreatedPoint,'',  0};
        Buttons^[06] := MBUTTON{ enabled, act.Counter,         'p', 1};
        Buttons^[07] := MBUTTON{ enabled, act.Watchpoint,      'w', 1};
        Buttons^[08] := MBUTTON{ separator };
        Buttons^[09] := MBUTTON{ enabled, act.Access,    'a', 1};
        Buttons^[10] := MBUTTON{ enabled, act.Condition, 'c', 1};
        Buttons^[11] := MBUTTON{ separator };
        Buttons^[12] := MBUTTON{ enabled, act.Disable,   '',  0};
        Buttons^[13] := MBUTTON{ enabled, act.Enable,    '',  0};
        Buttons^[14] := MBUTTON{ enabled, act.Delete,    '',  0};
        Buttons^[15] := MBUTTON{ separator };
        Buttons^[16] := MBUTTON{ enabled, act.ViewAll,   'v', 1};
        Buttons^[17] := MBUTTON{ enabled, act.DisableAll,'',  0};
        Buttons^[18] := MBUTTON{ enabled, act.EnableAll, '',  0};
        Buttons^[19] := MBUTTON{ enabled, act.EraseAll,  '',  0};
       <* END *>
