...
PROCEDURE gen_type_cast * (bs-: ARRAY OF CHAR; t: pc.STRUCT; f:=NIL: pc.STRUCT);
VAR str: STR;
    t_size, f_size: LONGINT;
BEGIN
  dcl.type_designator(str,bs,t);
  out.wr('('); out.ws(str); out.wr(')');

  IF (f # NIL) & (t.mode IN pc.CARDs)
   & (f.mode IN (pc.INTs + pc.TY_SET{pc.ty_loc}))
  THEN
    t_size := pc.code.get_size(pc.su_bytes, t);
    f_size := pc.code.get_size(pc.su_bytes, f);
    IF t_size > f_size THEN
       CASE f.mode OF
       | pc.ty_loc
       , pc.ty_shortint:  t := pc.shortcard_type;
       | pc.ty_integer:   t := pc.cardinal_type;
       | pc.ty_longint:   t := pc.longcard_type;
       ELSE               t := NIL;
       END;
       IF t # NIL THEN
         dcl.type_designator(str, "", t);
         out.wr('('); out.ws(str); out.wr(')');
       END;
    END;
  END;
...
*)
	ELSE
          IF (ll.mode = pc.nd_value) & (ll.type.mode IN pc.ARRs) 
           & ((t.obj = NIL) OR t.obj.is_public())
          THEN
            gen_type_cast("*", dcl.get_base_type(ll.type));
          END;
...
BEGIN
  ...
  CASE n.sub OF
	...
	IF p>=10 THEN out.wr(')') END;
      ELSE
        CASE n.type.mode OF
        |pc.ty_shortint,pc.ty_shortcard,pc.ty_char,pc.ty_loc  : i:= 8;
        |pc.ty_integer,pc.ty_cardinal                         : i:=16;
        |pc.ty_longint,pc.ty_longcard,pc.ty_ZZ                : i:=32;
--        |pc.ty_longlongint,pc.ty_longlongcard                 : i:=64;
        |pc.ty_set                                            : i:=n.type.len;
        END;
	nms.x2c(nms.nm_ash, nm, i); out.wf("%s(", nm);
	...
	gen_value(n.r,0,{}); out.wr(')');
      ELSE
	nms.x2c(nms.nm_rot_set, nm, n.type.len); out.wf("%s(", nm);
...
--          |pc.ty_longlongint                    : i:=64;
	...
	IF p>=8 THEN out.wr(')') END;
      ELSIF chk_neg() THEN
	IF b THEN nms.x2c(nms.nm_inl,nm) ELSE nms.x2c(nms.nm_in, nm, t.len) END;
	out.wf("%s(",nm);
	l:=n.l;
...
BEGIN
  ...
  THEN (* 1 .. 4 *)
    ...
    ELSE
      ...
      WHILE (nf.mode = pc.nd_unary)
          ...
          & (  ((nf.type.mode IN pc.CARDs) & (nf.l.type.mode IN pc.CARDs))
            OR ((nf.type.mode IN pc.INTs) & (nf.l.type.mode IN pc.INTs))
            )
      DO
  ...
  ELSIF (sf.mode IN pc.WHOLEs + pc.TY_SET{pc.ty_loc} - pc.TY_SET{pc.ty_ZZ})   -- from WHOLEs
      & (st.mode IN pc.ORDs + pc.TY_SET{pc.ty_loc})    -- to ORDinals   range check possible
  THEN (* 1 .. 4 *)
    IF dcl.c_types_equ(sf, st, TRUE) THEN
      gen_value(n.l, p, md);
    ELSE
      nf := n.l;
      WHILE (nf.mode = pc.nd_unary)
...
BEGIN
  ...
  REPEAT
    ...
    IF l.mode=pc.nd_node THEN
      nms.x2c(nms.nm_set_range, nm, n.type.len); out.wf("%s(",nm);
      f:=l.l; t:=l.r;
      gen_value_minus_min(f,0,n.type.min,TRUE);
      out.wr(',');
      gen_value_minus_min(t,0,n.type.min,TRUE);
