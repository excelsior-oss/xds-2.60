...
(** name's indexes *)
  ...
  nm_ash16       *=102;
  nm_ash8        *=103;
  nm_Number       =104;  -- number of nm_ constant it's used in VAR "names" 
...
PROCEDURE x2c*(n: INTEGER; VAR nm: ARRAY OF CHAR; len:=32: LONGINT);
BEGIN
  IF n = nm_ash THEN
    CASE len OF
    | 32:
...
BEGIN
  ...
  std(nm_ash16,"X2C_ASH16");
  std(nm_ash8, "X2C_ASH8");
END ini_sys_tree;
