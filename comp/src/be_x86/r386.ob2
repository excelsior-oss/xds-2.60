...
<* END *>
    ...
    ELSIF srcreg <> r THEN
        ...
        reg.MarkVarInReg( reg.regContents [srcreg], r );
    END;
    reg.OccupyReg(r);
END MoveRegReg;
...
--    hookedRegsV := reg.HookedRegsWOVar (v, src);
    ...
    IF RD.Loc^[v].tag = NTreg THEN
        r := RD.Loc^[v].reg;
(* JIRA issue XDS-55: 
-- +nooptimize can leads to have 'l' with 'ir.o_par' and 'ir.y_AddrConst'.
-- In this case 'l.par.name' is a index inside the 'ir.Locals' not in the 'ir.Vars'.
-- As result we have invalid index at 'HookedRegsWOVar' of reg386.ob2.
-- To reproduce the issue try to use in nested procedure local variable of its parent procedure.  
...
--                reg.MarkUseReg (l^.place);
            ...
            IF reg.regContents [t]# ir.UNDEFINED THEN
              reg.MarkVarInReg( reg.regContents [t], D.RegTable[D.ECXp,ir.Vars[reg.regContents [t]].Def.ResSize] );
              reg.FillUsagesLeft  (D.RegTable[D.ECXp,ir.Vars[reg.regContents [D.ECX]].Def.ResSize], reg.usagesLeft[t]);
            END;
            IF reg.usagesLeft [D.ECX] = 0 THEN
                reg.FreeReg(D.ECX);
            ELSE
                reg.OccupyReg(D.RegTable[D.ECXp,ir.Vars[reg.regContents [D.ECX]].Def.ResSize]);
            END;
        END;
