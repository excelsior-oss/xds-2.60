...
(* --- Switches to dis/enable some analysis & optimizations ---
  ...
  NeverInline  : BOOLEAN; -- completely disable procedure inlining, even "AlwaysInline"
                          --   M: = "NEVERINLINE"     V: = FALSE
...
PROCEDURE IsInVals ( o: pc.OBJECT
                   )  :           BOOLEAN;
(**
...
VAR
  ...
  BEGIN
    WHILE ((nn.mode = pc.nd_unary) AND ((nn.sub = pc.su_cast) OR (nn.sub = pc.su_conv))) OR
          (nn.mode = pc.nd_guard) OR
          (nn.mode = pc.nd_aggregate) DO nn := nn.l; END;
    RETURN (nn.mode = pc.nd_value);
  END is_val;
  ----
...
BEGIN
    IF ~NeverInline
       & ~(pc.ttag_neverinline IN o.type.tags)
...
(*
*)
VAR nodes : ARRAY 4096 OF pc.NODE;
    ncnt  : INTEGER;
...
<* END *>
  ...
  ELSE
    IF (n.mode = pc.nd_call)
     & (pcS.oberon & pcS.ext() & (env.o2_spaghetti IN env.config.tags))
    THEN
      IF GenCall (n(*=>*)) THEN
        GenDesignator (n, addr, tags);
      END;
      ASSERT( n.type.mode # pc.ty_void);
    ELSE
...
BEGIN
  ...
  NeverInline  := env.config.Option ("NEVERINLINE");
  EnSL1Addr    := pc.code.name = "STAT";
